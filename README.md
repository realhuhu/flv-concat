...
- AAC 音频
- 多文件必须具有相同的分辨率、编码、采样率和声道数。H.264 比较 NAL 长度、SPS 和 PPS；H.265 比较 NAL 长度、SPS 的 profile/色度/位深/尺寸及参数集 ID，允许编码器在分段时重新生成 VPS/SPS/PPS 的参考帧、VUI 和默认切片参数，并在每个关键帧前无损注入新参数集，保证顺序播放和随机跳转都可解码；AAC 比较对象类型、采样率、声道布局和帧长。无法通过这种方式安全调整的格式变化会在初始化阶段拒绝合并。
